...
// tcpco.bpf.c - TCP Congestion Observatory (versão simplificada)
...
struct tcp_metrics {
    __u32 snd_cwnd;
    __u32 ssthresh;
    __u32 srtt;            // srtt_us >> 3 (microssegundos)
    __u32 retransmissions; // total_retrans
    __u32 duplicate_acks;  // packets_out (proxy p/ pacotes em voo)
    __u64 bytes_acked;
    __u32 ca_state;        // icsk_ca_state (Open/Disorder/CWR/Recovery/Loss) - bônus
    __u64 ts_ns;           // timestamp do evento
};

// Requisito: pelo menos um mapa eBPF por conexão -> hash indexado por flow_key.
struct {
...
int BPF_KPROBE(handle_tcp_rcv, struct sock *sk)
{
    ...
    // Só IPv4 nesta versão simplificada.
    ...
    key.src_port = BPF_CORE_READ(sk, __sk_common.skc_num);           // host order
